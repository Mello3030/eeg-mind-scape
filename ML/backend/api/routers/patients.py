...
def create_patient(payload: PatientCreate, db: Session = Depends(get_db)) -> PatientDetail:
...
def list_patients(
    ...
    patients, total = crud.list_patients(db, search, limit, offset)
...
def get_patient(patient_id: str, db: Session = Depends(get_db)) -> PatientDetail:
    return _detail(db, _get_or_404(db, patient_id))
