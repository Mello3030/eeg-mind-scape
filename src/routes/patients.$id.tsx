...
import { ClassBadge, EmptyState, Kpi, Panel, StatusBadge, fmtDate } from "@/components/ui-kit";
import { tooltipStyle } from "@/components/views/DashboardView";
import { CLASSES, STREAMS, classColor, pct } from "@/lib/qsfe";
import { analysesForPatient, getPatient, listRecordings, updatePatient, useDb } from "@/services/mockApi";
...
export const Route = createFileRoute("/patients/$id")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Patient EEG recordings, QSFE-Net prediction history, probability trajectory and gate-weight history.",
      },
      { property: "og:title", content: "Patient Record — QSFE-Net EEG History" },
      { property: "og:description", content: "Longitudinal EEG screening history for one patient." },
    ],
  }),
...
function PatientPage() {
  ...
  const data = useDb(() => {
    ...
    return {
      patient,
      recordings: patient ? listRecordings(patient.id) : [],
      analyses: patient ? analysesForPatient(patient.id) : [],
    };
  });
  ...
  return (
    <AppShell
      title={`Patient ${p.externalPatientId}`}
      subtitle={`${p.age} y · ${p.sex} · registered ${fmtDate(p.createdAt)}`}
      actions={
        <Link to="/patients" className="flex items-center gap-1 rounded border border-border px-2.5 py-1.5 text-xs">
          <ArrowLeft className="size-3.5" /> All patients
        </Link>
      }
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Kpi label="Recordings" value={data.recordings.length} />
      ...
      <div className="mt-3 grid gap-3 xl:grid-cols-[1fr_1.4fr]">
        <Panel title="Patient information">
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-[11px]">
            <Item k="External patient ID" v={p.externalPatientId} />
            <Item k="Internal ID" v={p.id} />
            <Item k="Age" v={String(p.age)} />
            <Item k="Sex" v={p.sex} />
          </dl>
          ...
          <button
            onClick={() => {
              updatePatient(p.id, { notes: notes ?? p.notes });
              setNotes(null);
            }}
            className="mt-2 rounded border border-border px-2.5 py-1 text-[11px]"
          >
            Save notes
          </button>
        </Panel>
      ...
      <div className="mt-3 grid gap-3 xl:grid-cols-2">
            ...
            <tbody>
              {data.recordings.map((r) => (
                <tr key={r.id} className="border-b border-border/70 last:border-0">
                  <td className="num py-2 pr-3">{r.filename}</td>
                  <td className="num py-2 pr-3">{r.samplingRate} Hz</td>
                  <td className="num py-2 pr-3">{r.channels}</td>
                  <td className="num py-2 pr-3">{r.duration} s</td>
                  <td className="py-2 pr-3">
                    <StatusBadge status={r.status} />
                  </td>
                  <td className="num py-2 pr-3 text-muted-foreground">{fmtDate(r.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Panel title="Gate-weight history" hint="Stream contribution per session">
          {history.length === 0 ? (
            <EmptyState title="No gate weights" body="Gate weights appear once an analysis completes." />
          ) : (
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={history} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
